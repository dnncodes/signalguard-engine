      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {signals.length === 0 ? (
          <div className="col-span-full h-64 flex flex-col items-center justify-center border border-dashed border-engine-border rounded-lg text-engine-text-dim">
            ...
            <p className="text-[10px] mt-2 text-engine-text-muted">Next signal in ~5 minutes</p>
          </div>
        ) : (
          signals.map((signal) => (
            <div
              key={signal.id}
              className="group relative overflow-hidden p-5 engine-panel engine-panel-hover rounded-lg transition-all duration-300"
            >
              {/* Accent bar */}
              <div
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div className="p-3 bg-engine-bg rounded-md border border-engine-border">
                  <p className="text-[9px] text-engine-text-dim uppercase font-bold mb-1">Score</p>
              ...
              <div className="flex items-center gap-2 text-[11px] text-engine-text-secondary">
                <Target size={12} className="text-signal-buy flex-shrink-0" />
